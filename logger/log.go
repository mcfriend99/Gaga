...
type Config struct {
	// AppName refers to the name of the calling application
	AppName string
...
// Should you need to create multiple Logger objects, better to associate them with different directories, at least with different filename prefixes,
...
func New(cfg *Config) (logger *Logger, err error) {
	...
	err = logger.initLoggerImpl(cfg.LogFilenamePrefix)
	if err != nil {
		logger = nil
	}
	return
...
func (l *Logger) initLoggerImpl(filenamePrefix string) (err error) {
	if len(filenamePrefix) == 0 {
		filenamePrefix = "%P.%U" // Default value
	}
...
func (l *logger) log(t time.Time, data []byte) {
	...
	if !l.closed {
		if l.size >= l.parent.logFileMaxSize || l.day != d || l.file == nil {
			...
			filename := fmt.Sprintf("%s$%s@%d-%02d-%02dT%02d%02d%02d.%06d.log", l.parent.logPathPrefix, kLogLevelNames[l.level],
